...
uint8_t   dbus_buf[DBUS_BUFLEN];//如何实现的呢？
...
void dbus_uart_init(void)//DBUS串口初始化
{
	...
	//HAL_UART_Receive_DMA(&huart3,dbus_buf, sizeof(dbus_buf));
}
    
void rc_callback_handler(rc_info_t *rc, uint8_t *buff)
{//用于处理接收到的遥控器（rc）数据，将接收到的字节解码
  rc->ch0 = (buff[0] | buff[1] << 8) & 0x07FF;//将buff[0]和buff[1]的值组合为ch0通道的值，并将其限制在11位（通过与0x07FF按位与）==11111111111
