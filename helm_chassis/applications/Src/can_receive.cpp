...
/*******对象*******/
CAN_BUS can_bus;
moto_info_6020_t motor_info[7];
#define FEEDBACK_ID_BASE 0x205

void gm6020_cmd(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4) // can2 gm6020
{
...
void CAN_BUS::CMD::CAN1_6020(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4)
{
...
void CAN_BUS::CMD::CAN1_3508(int16_t motor5, int16_t motor6, int16_t motor7, int16_t motor8)
{
 uint8_t	can_send_data[8];
 uint32_t send_mail_box;
 CAN_TxHeaderTypeDef can_tx_message;
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	if(hcan->Instance==CAN1)
	{
		...
		if ((rx_header.StdId >= FEEDBACK_ID_BASE) && (rx_header.StdId < FEEDBACK_ID_BASE + 7))
		{
			
		volatile static uint8_t i = 0;
		i = rx_header.StdId - FEEDBACK_ID_BASE;
		can_bus.dji_encoder.get_motor_measure_6020(&can_bus.motor_can1_6020[i], rx_data_1,i);
		...
		switch (rx_header.StdId)
		{
			...
			case CAN_3508_M4_ID:
			{
				...
				can_bus.motor_can1_3508[i].msg_cnt++ <= 50 ?	
				can_bus.dji_encoder.get_moto_offset(&can_bus.motor_can1_3508[i], rx_data_1) : 
				can_bus.dji_encoder.get_motor_measure(&can_bus.motor_can1_3508[i], rx_data_1);
				can_bus.dji_encoder.get_motor_measure(&can_bus.motor_can1_3508[i], rx_data_1);
				break;
			}
			case CAN_3508_M5_ID:
...
//		if ((rx_header.StdId >= FEEDBACK_ID_BASE) && (rx_header.StdId < FEEDBACK_ID_BASE + 7)) // judge the can id
//	{
...
//		if (index == 1) // 对对应的6020设置0位置的偏移量
//		{

//			motor_info[index].rotor_angle += 8191 /3;
//		}
//		
//		if (index == 0) // 对对应的6020设置0位置的偏移量
//		{

//			motor_info[index].rotor_angle += 8191*5/6 ;
//		}
//		if (index == 2) // 对对应的6020设置0位置的偏移量
//		{

//			motor_info[index].rotor_angle += 8191 *7/ 12;
//		}

//		if (motor_info[index].rotor_angle > 8191)
//		{
//			motor_info[index].rotor_angle -= 8191;
//		}
//	}
...
void CAN_BUS::DJI_ENCODER::get_motor_measure_6020(motor_measure_t *ptr,uint8_t data[],uint8_t i)                                                     
{	
...
//				((ptr)->angle) = (int32_t)(((ptr)->ecd) - ((ptr)->last_ecd));
		...
		if(i==0)
		{
			ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - 731;
		}
		else if(i==1)	
		{
			ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - 1409;
		}
		else if(i==2)
		{
			ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle - 4157;
		}
		else if(ptr->total_angle>8191)
		{
			ptr->total_angle-=8191;
		}
}
